...
export function useNavigationPanel(
  ...
  // Fix: Create stable event handlers without Animated.event
  const onPageScroll = useCallback((event: { nativeEvent: PagerViewOnPageScrollEventData }) => {
  ...
  }, [onPageScrollOffset, onPageScrollPosition, addLog]);

  const onPageSelected = useCallback((event: { nativeEvent: PagerViewOnPageSelectedEventData }) => {
  ...
  }, [onPageSelectedPosition, addLog, onPageSelectedCallback]);
